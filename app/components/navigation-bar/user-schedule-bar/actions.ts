...
import { addDays, addHours, addMinutes, addYears } from "date-fns"

export async function addScheduleAction(never: string[], modifiable: string[]) {
  ...
  const after1Year = addYears(indexDate, 1)

  while (indexDate <= after1Year) {
